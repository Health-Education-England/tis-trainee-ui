...
interface ICOJAug2022 {
  isSpecialty: boolean;
}

const COJAug2022: React.FC<ICOJAug2022> = ({ isSpecialty }) => {
  const trainingProg = store.getState().traineeProfile.viewedCojProg;
  console.log("training prog: ", trainingProg);

  if (!trainingProg) {
    return <Redirect to="/profile/programmes" />;
  }

  const trainingProgText = isSpecialty
    ? `${trainingProg?.programmeName}`
    : `${trainingProg?.programmeName} Foundation School`;
  const exclusionOrSuspension = isSpecialty ? "exclusion" : "suspension";
  const whatBody = isSpecialty ? "MTPS or GMC/GDC" : "GMC, GDC or NHS England";
  const furtherAction = isSpecialty
    ? ", or if NHS England take action to restrict my ability to work as a doctor or a dentist"
    : "";
  const preChecks = isSpecialty
    ? "pre-employment checks and occupational health assessments, "
    : "";
  const pdOther = isSpecialty
    ? ""
    : "e) I understand that I will be contacted by HEE/NES/the Wales Deanery/NIMDTA about matters that are relevant to my teaching, training or personal development.";
  return (
    <>
      <Card feature>
        <Card.Content>
          <Card.Heading>
            {`${trainingProg?.programmeName} Specialty Training Programme - Conditions of Joining agreement`}
          </Card.Heading>
          <SummaryList noBorder>
            ...
            <SummaryList.Row>
              <SummaryList.Value>
                ...
                {trainingProgText}, I agree to meet the following requirements
            ...
            <SummaryList.Row>
              <SummaryList.Value>
                ...
                police investigation, immediate {exclusionOrSuspension} from
                employment and referral to the GMC. Failure to do so may also
                result in my removal from the training programme.
              </SummaryList.Value>
            </SummaryList.Row>
            <SummaryList.Row>
              <SummaryList.Value>
            ...
            <SummaryList.Row>
              <SummaryList.Value>
                • I will inform my Responsible Officer, HEE/NES/HEIW/NIMDTA and
                my employer immediately if the {whatBody} place any conditions
            ...
            <SummaryList.Row>
              <SummaryList.Value>
                ...
                familiarity with policies, participating in {preChecks} employer
                and departmental inductions, and workplace-based appraisal as
                well as educational appraisal. I acknowledge and agree to the
                need to share information about my performance as a doctor in
                training with other organisations (e.g. employers, medical
                schools, the GMC, Colleges/training bodies involved in my
