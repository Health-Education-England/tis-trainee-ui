...
export async function markAsUnreadBE(row: NotificationType) {
  ...
  // // TODO check if we want to keep this optimistic update/rollback logic
...
// Note: we can refactor this later to make more generic when we need to account for placement/:placementId links etc.
...
export function resolveInternalTrackerLink(
  ...
  const notificationId = notificationMap.get(tag as NotificationSubjectType); // TODO fudged Type cast here for now
