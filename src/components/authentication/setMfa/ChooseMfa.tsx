...
const ChooseMfa = ({ mfa }: IChooseMfa) => {
  ...
  useEffect(() => {
    dispatch(resetError());
    dispatch(resetBothSections());
    dispatch(updatedTotpCode(""));
  }, [dispatch]);
  ...
  return (
    <Formik
      ...
      onSubmit={values => {
        dispatch(updatedCurrentMfa(values.mfaChoice));
    ...
    >
      {({ handleSubmit }) => (
        <>
          ...
          <Details>
            <Details.Summary data-cy="mfaSummary">What is MFA?</Details.Summary>
            <Details.Text data-cy="mfaText">
              <p>
                Multi factor authentication (MFA) provides a second layer of
                security. When enabled, each time you log in to TIS Self-Service
                after entering your password, you'll be asked for a new{" "}
                <b>6-digit code to verify your identity</b> using either of the
                following:
              </p>
