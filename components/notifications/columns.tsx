...
import { faCheck, faEnvelope, faEnvelopeOpen, faTriangleExclamation } from "@fortawesome/free-solid-svg-icons";
...
export const emailColumns = [
  columnHelper.accessor("status", {
    ...
    cell: props => {
      const statusClass =
        props.row.original.status === "FAILED" ? "status-failed" : "status-sent";
      return (
        <span className={`${statusClass} nhsuk-margin-left-1`}>
  ...
  columnHelper.accessor("contact", {
    id: "contact",
    header: ({ column }) => <TableColumnHeader column={column} title="Sent to" />,
    cell: props => <span>{props.renderValue()}</span>,
    enableColumnFilter: false
  }),
]
